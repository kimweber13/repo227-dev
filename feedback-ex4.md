...
- [x] (12 / 13 Punkte) View zur Verwaltung von `ToDos`
  ...
  - [.] 1 (2 Punkte) View-Elemente für `PUT /todos/$id` // Assignees werden nicht in Liste übernommen
...
- [ ] (1 Punkt) Gängige UI Components werden passend eingesetzt (z.B. Tables, MultiSelects, DatePicker, etc.) // Man muss für ein Datum auch eine Uhrzeit auswählen
...
- [ ] (1 Punkt) Repository-Struktur folgt dem Beispielprojekt (`api` und `frontend` auf höchster Ebene)
- [x] (1 Punkt) Keine IDE-spezifischen Dateien (z.B. `.idea`, `.vscode`, etc.) oder Binaries / Libraries (z.B. `target`, `node_modules`, etc.) im Repository // package.json im root
